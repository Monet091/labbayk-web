<?php
    ...
    // Pengaturan untuk server lokal (XAMPP/Laragon)
        $host = "localhost";
        $user = "root";       // Default username XAMPP adalah 'root'
        $password = "";       // Default password XAMPP adalah kosong
        $dbName = "labbayk_hris"; 

        try {
            // Koneksi PDO standar tanpa port khusus dan tanpa SSL
            $koneksi = new PDO("mysql:host=$host;dbname=$dbName", $user, $password);
            $koneksi->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            
            // INGAT: Jangan pernah menaruh echo "Koneksi Berhasil" di sini 
            // karena akan merusak format JSON saat proses login!
